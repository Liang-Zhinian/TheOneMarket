...
export default class BaseModel {
  ...
  static defaultUrl = 'http://demo.atpath.com:5216/wordpress/wp-json/wc/v3/'
  api = API

  baseUrl = global.baseUrl || 'http://demo.atpath.com:5216/wordpress/wp-json/wc/v3/'
  ...
  consumerToken = {
    consumerKey: "ck_1cfa5cc5cbc7746680f1b3cbd4f5daa690f2da92",
    consumerSecret: "cs_c56c571c531510a010a5b95cb9f0264726a0d1d7"
  }
  ...
  async apiReq(type, { optionalParams, options }) {
    ...
    result = await this.sendRequest(this.request.method, api.url, this.request.optionalParams)
  ...
  async sendRequest(method, endpoint, optionalParams) {
    method = method.toLowerCase()
    if (optionalParams) {
      Object.keys(optionalParams).map(key => {
        var regex = new RegExp('{' + key + '}', "g");
        endpoint = endpoint.replace(regex, optionalParams[key])
      })
    }
    console.log(`NEW REQUEST { ${method} } TO =>`, endpoint, optionalParams)
    let res = null
    try {
      switch (method) {
        case 'get':
          res = await WooCommerceAPI.get(endpoint, optionalParams);
          break;
      ...
      res.next_page = res;
      res.prev_page = res;
      res.current_page = res;
      res.total_data = res.length;
    } catch (e) {
      res.status = 'error';
    }
